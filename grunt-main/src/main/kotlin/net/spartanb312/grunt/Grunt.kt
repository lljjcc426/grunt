...
import net.spartanb312.grunt.web.WebServer
...
const val SUBTITLE = "build 241225"
...
fun main(args: Array<String>) {
    ...
    // Check for --gui flag (legacy Swing mode) or --web flag
    val useSwingGui = args.any { it == "--gui" }
    val useWeb = args.any { it == "--web" }
    val webPort = args.firstOrNull { it.startsWith("--port=") }?.substringAfter("=")?.toIntOrNull() ?: 8080
    ...
    // Web mode: start Ktor web server (default when no args)
    if (!useSwingGui && (useWeb || (guiMode && args.isEmpty()))) {
        try {
        ...
        WebServer.start(webPort, config)
        return
    }
